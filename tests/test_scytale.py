...
class scytaleTests(unittest.TestCase):
    ...
    def test_equal_io_encode(self):
        ...
        """
        self.assertEqual(len(scytale_init("hallo sipgate", 1, 0)),
                         len("hallo sipgate"))
        self.assertEqual(len(scytale_init("hallo welt!", 6, 0)),
                         len("hwaelllto! "))

    def test_seq_encode_min(self):
        """sequence with diameter 1 should result in the same output"""
        self.assertEqual((scytale_init("sipgate hacking talents!", 1, 0)),
                         "sipgate hacking talents!")
        self.assertEqual((scytale_init("hallo welt!", 1, 0)), "hallo welt!")

    def test_even_seq_encode(self):
        """sequence with diameter > 1 should decode seq"""
        self.assertEqual((scytale_init("sipgate hacking talents!", 6, 0)),
                         "seili nephgnga tactstka!")
        self.assertEqual((scytale_init("sipgate hacking talents!", 5, 0)),
